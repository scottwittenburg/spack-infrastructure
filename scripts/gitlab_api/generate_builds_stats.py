...
the current working directory.""")
    ...
    try:
        ...
        for pipeline in pipelines:
            ...
            if db_cursor:
                db_cursor.execute("INSERT OR IGNORE INTO pipelines VALUES (?, ?, ?, ?);", (pipeline_id, pipeline['web_url'], pr_number, branch))
            ...
            for p_job in pipeline_jobs:
                ...
                with open(generated_pipeline_yaml) as fd:
                    pipeline_jobs = yaml.safe_load(fd.read())

                for job_name, job_definition in pipeline_jobs.items():
